...
const DEFAULT_INPUT_PATH = path.resolve(__dirname, '../data/vehicle-vmmr-readiness.json');
const DEFAULT_OUTPUT_PATH = path.resolve(__dirname, '../data/vehicle-vmmr-readiness.json');

const DEFAULT_POLICY = {
    noMakeModelWithoutValidatedModel: true,
    activationMetric: 'mAP50',
    activationThreshold: 0.85,
    notes: 'Fine-grained make/model/body-type output remains disabled until an active model report passes this gate.',
};
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--input') {
            ...
            console.log('Usage: node scripts/build-vehicle-vmmr-readiness.js [--check|--write] [--input data/vehicle-vmmr-readiness.json] [--output data/vehicle-vmmr-readiness.json] [--allow-fixture]');
            process.exit(0);
        } else {
...
function normalizePolicy(policy) {
    ...
    const activationThreshold = typeof source.activationThreshold === 'number'
        ? source.activationThreshold
        : DEFAULT_POLICY.activationThreshold;

    return {
        noMakeModelWithoutValidatedModel: source.noMakeModelWithoutValidatedModel ?? DEFAULT_POLICY.noMakeModelWithoutValidatedModel,
...
function normalizeReadiness(input) {
    ...
    return {
        ...
        taxonomy: clean(source.taxonomy) || 'vehicle_vmmr_readiness_v1',
        policy: normalizePolicy(source.policy),
        datasets,
        modelReports,
        active_model_count: activeModelCount,
        fine_grained_model_ready: activeModelCount > 0,
        ...
        validation_note: clean(source.validation_note)
            || 'Fine-grained make/model/body-type output remains disabled until an active model report passes this gate.',
    };
}

function buildVehicleVmmrReadiness(input, options = {}) {
...
function run() {
    ...
    const readiness = buildVehicleVmmrReadiness(input, { allowFixture: options.allowFixture });

    if (options.write) {
        fs.writeFileSync(options.outputPath, `${JSON.stringify(readiness, null, 2)}\n`, 'utf8');
    }

    console.log(`ok - vehicle VMMR readiness builder ${readiness.active_model_count > 0 ? 'ready' : 'pending'} (${readiness.active_model_count} active models)`);
}
